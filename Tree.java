...
public class Tree {
	...
	public static void main(String[] args) {
		...
		System.out.println(numsOfkLevelTreeNode(root, 3));
		...
		System.out.println(PrintTreeByLevel(root));
		...
		System.out.println(preOrderReverse(root));

		System.out.println(inOrder(root));

		System.out.println(postOrder(root));

		System.out.println(insertNode(root, root2));

		findPath(root, 2);
	}

	// 打印二叉树
	public static ArrayList PrintTreeByLevel(TreeNode root) {
	...
	// 二叉树的搜索区间
	// 给定两个值 k1 和 k2（k1 < k2）和一个二叉查找树的根节点。找到树中所有值在 k1 到 k2 范围内的节点。即打印所有x (k1 <=
	// x <= k2) 其中 x 是二叉查找树的中的节点值。返回所有升序的节点值。
	...
	private static Result getMaxDistanceResult(TreeNode root) {
		if (root == null) {
	...
	//不同的二叉树   给出 n，问由 1...n 为节点组成的不同的二叉查找树有多少种
